            ...
            <div className="flex items-center gap-2 shrink-0">
              <button
                onClick={async () => {
                  ...
                  setIsIndexing(true);
                  try {
                    const res = await fetch("/api/audio/embed-song/", {
                      method: "POST",
                    ...
                    if (res.ok) {
                      toast.success(`埋め込み更新完了: ${data.updated}件`);
                    } else {
                      toast.error(`埋め込み更新失敗: ${data.error}`);
                    }
                  } catch {
                    toast.error("埋め込み更新に失敗しました");
                  } finally {
                    setIsIndexing(false);
                  }
                }}
                ...
                title="MuQ埋め込みが未登録の楽曲をインデックスに追加します"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
                </svg>
                {isIndexing ? "更新中..." : "埋め込み更新"}
              </button>
              <button
